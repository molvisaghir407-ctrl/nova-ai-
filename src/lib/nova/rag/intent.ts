...
const INTENT_PATTERNS: Array<{ intent: QueryIntent; patterns: RegExp[] }> = [
  { intent: 'weather', patterns: [/\b(weather|temperature|rain|snow|forecast|humidity|wind speed|feels like|air quality|uv index|storm|thunder)\b/i] },
  { intent: 'finance', patterns: [/\b(stock|share price|market cap|crypto|bitcoin|ethereum|btc|eth|nasdaq|nyse|s&p|dow jones|earnings|revenue|ipo|dividend|forex|exchange rate|gold price|oil price)\b/i] },
  { intent: 'news', patterns: [/\b(latest|breaking|today|this week|this month|just announced|recently|2024|2025|2026|current|live|right now|score|standings|headline|update|happening|trending)\b/i] },
  { intent: 'sports', patterns: [/\b(score|match|game|tournament|championship|league|nba|nfl|premier league|fifa|ipl|cricket|tennis|formula 1|f1|player stats|transfer|squad)\b/i] },
  {
    intent: 'code',
    patterns: [
      /```|```[^`]+```/,
  ...
  { intent: 'science', patterns: [/\b(physics|chemistry|biology|genetics|quantum|relativity|thermodynamics|evolution|dna|rna|protein|atom|molecule|element|periodic table|astronomy|astrophysics|geology|ecology|neuroscience|immunology)\b/i] },
  { intent: 'medical', patterns: [/\b(symptom|diagnosis|medication|treatment|disease|syndrome|disorder|doctor|hospital|surgery|therapy|clinical|pharmaceutical|vaccine|cancer|diabetes|hypertension|depression|anxiety|dosage|side effect)\b/i] },
  { intent: 'legal', patterns: [/\b(law|legal|court|lawsuit|contract|liability|regulation|compliance|patent|copyright|trademark|gdpr|terms of service|privacy policy|jurisdiction|statute|precedent)\b/i] },
  { intent: 'history', patterns: [/\b(history|historical|ancient|medieval|century|war|revolution|empire|civilization|dynasty|timeline|era|decade|founded|origin|invention|discovery)\b/i] },
  { intent: 'geography', patterns: [/\b(country|capital|city|continent|population|area|border|region|geography|map|location|coordinates|distance|timezone|language|currency of)\b/i] },
  { intent: 'comparison', patterns: [/\b(vs\.?|versus|compared? to|difference between|better than|pros and cons|advantages|disadvantages|which is|which should)\b/i] },
  { intent: 'howto', patterns: [/^how (to|do|does|can|should|would)\b/i, /\b(step by step|tutorial|guide|instructions|walkthrough|setup|install|configure)\b/i] },
  { intent: 'factual', patterns: [/^(who|what|when|where|which|how many|how much|is it true|define|meaning of|what is|what are)\b/i] },
  { intent: 'conversational', patterns: [/^(hi|hello|hey|thanks|thank you|ok|sure|bye|great|nice|cool|lol|haha|what'?s up|sup|yo|morning|evening|night|good day)\b/i] },
  { intent: 'creative', patterns: [/\b(write a|create a|generate a|make a|compose|poem|story|essay|song|haiku|script|creative|fiction|narrative|draft|brainstorm)\b/i] },
];
