...
export default makeAgent({
  ...
  run: async (query, signal) => {
    ...
    try {
      ...
      if (!res.ok) return [];
      const data = await res.json() as { current_condition?: Array<{ temp_C?: string; temp_F?: string; weatherDesc?: Array<{ value?: string }>; humidity?: string; windspeedKmph?: string }> };
      ...
      return [{ id: 1, title: `Weather in ${location}`, url: `https://wttr.in/${location}`, snippet: desc, domain: 'wttr.in', date: new Date().toISOString() }];
    } catch { return []; }
  },
});
